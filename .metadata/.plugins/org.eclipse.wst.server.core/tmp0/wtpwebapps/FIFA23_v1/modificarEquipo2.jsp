					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								Equipo equipo = conexion.dameEquipo(codEquipo);
								
								%>
								
								<form action="operaciones.jsp?tipo=modificarEquipo&codEquipo=<%=equipo.getCod_equipo() %>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input value="<%=equipo.getNombre() %>" type="text" name="nombreEquipo" id="nombreEquipo" placeholder="Nombre del equipo" style="width:450px"; maxlength="30"/>
										</div>
										
										<div class="">
											<label>Liga</label>
											<select id="id_select2_example" name="codLiga">
											<%
											String selected = "";
											
											for(int i = 0; i < ligas.size(); i++) {
												if (ligas.get(i).getCod_liga() == equipo.getCod_liga()) {
													selected = "selected";
												} else {
													selected = "";
												}
											%>
												<option data-img_src="images/ligas/<%=ligas.get(i).getCod_liga() %>.png" <%=selected %> value="<%=ligas.get(i).getCod_liga()%>"><%=ligas.get(i).getNombre() %></option>
											<%}; %>
											</select>
										</div>
