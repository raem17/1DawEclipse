					...
					<div class="container">
						<div id="content">
							...
							<!-- Content -->
								...
								ArrayList<Equipo> equipos = conexion.todosEquipos();
								Jugador juga = conexion.dameJugador(codJuga);
								%>
								
								<form action="operaciones.jsp?tipo=modificarJuga&codJuga=<%=juga.getCod_jugador() %>" method="post">
									<div class="row gtr-50">
										<div class="">
											<input type="text" name="nombreJuga" id="nombreJuga" placeholder="Nombre del jugador" style="width:450px"; maxlength="50" value="<%=juga.getNombre() %>"/>
										</div>
										<div class="">
											<label>Pierna</label>
											<select id="piernaJuga" name="piernaJuga">
												<%
												if(juga.getPierna().equalsIgnoreCase("Izquierda")) {
													out.print("<option value ='Izquierda' selected>Izquierda</option>");
													out.print("<option value ='Derecha'>Derecha</option>");

												} else if(juga.getPierna().equalsIgnoreCase("Derecha")) {
													out.print("<option value ='Izquierda'>Izquierda</option>");
													out.print("<option value ='Derecha' selected>Derecha</option>");
												} else {
													out.print("<option value ='Izquierda'>Izquierda</option>");
													out.print("<option value ='Derecha'>Derecha</option>");
												}
										...
										<div class="">
											<label>Equipo</label>
											<select id="id_select2_example" name="codEquipo">
											<%
											for(int i = 0; i < equipos.size(); i++) { 
												String selected = "";
												if(equipos.get(i).getCod_equipo() == juga.getCod_equipo()) {
													selected = "selected";
												} else {
													selected = "";
												}
											%>
												<option data-img_src="images/equipos/<%=equipos.get(i).getCod_equipo() %>.png" <%=selected %> value="<%=equipos.get(i).getCod_equipo()%>"><%=equipos.get(i).getNombre() %></option>
											<%}; %>
											</select>											
										</div>
										<div class="">
											<input value="<%=juga.getPais() %>" type="text" name="paisJuga" id="paisJuga" placeholder="País del jugador" maxlength="20"/>
										</div>
										<div class="">
											<input value="<%=juga.getAltura() %>" type="number" name="alturaJuga" id="alturaJuga" placeholder="Altura del jugador" maxlength="11"/>
										</div>
