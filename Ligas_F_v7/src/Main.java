...
public class Main {
	...
	// Metodo aux
	static boolean existeArbitro(String dni, ArrayList<Estadis_Arbitro> datosArbitros) {
		boolean existeArbitro = false;
		
		for (int i = 0; i < datosArbitros.size(); i++) {
			if (datosArbitros.get(i).getDni().equalsIgnoreCase(dni)) {
				existeArbitro = true;
				break;
			}
		}
		
		return existeArbitro;
	}
	
	// Metodo aux
	static boolean existeJugador(String dni, ArrayList<Jugador> datosJugadores) {
		boolean existeJugador = false;
		
		for (int i = 0; i < datosJugadores.size(); i++) {
			if (datosJugadores.get(i).getDni().equalsIgnoreCase(dni)) {
				existeJugador = true;
				break;
			}
		}
	...
	// Metodo aux
	static boolean existeMarcador (String marcador, ArrayList<Estadis_Marcador> datosMarcadores) {
		boolean existeMarcador = false;

		for (int i = 0; i < datosMarcadores.size(); i++) {
			if (datosMarcadores.get(i).getMarcador().equalsIgnoreCase(marcador)) {
				existeMarcador = true;
				break;
			}
		}
		
		return existeMarcador;
	}
	
	// Metodo aux
	static boolean existeEArbitrado (String nombre, ArrayList<Equipo_Arbitrado> datosEArbitrados) {
		boolean existeEArbitrado = false;
		
		for (int i = 0; i < datosEArbitrados.size(); i++) {
			if (datosEArbitrados.get(i).getNombre().equalsIgnoreCase(nombre)) {
				existeEArbitrado = true;
				break;
			}
		}
		
		return existeEArbitrado;
	}
}
